...
# Define weights for model predictions
...
district_mapping = {
    ...
    'BEDOK': 'East',
    # ... (add more towns and districts)
}

# Define a function to create a card with label and value
...
# Define a function to calculate HDB loan details
def calculate_hdb_loan_details(purchase_price, tenure_years, grants):
    ...
    # Calculate monthly breakdown of principal and interest
    breakdown = []
...
# Define a function to calculate Stamp Duty
...
# Sidebar for selecting HDB details
...
# Sidebar for selecting year and month of purchase
...
# Sidebar for selecting town, flat type, house floor, floor area, and lease commencement date
...
st.session_state['floor_area_sqm'] = st.sidebar.number_input('Enter floor area (square meters)')
...
# Button to evaluate HDB housing
if st.sidebar.button('Evaluate', key='Evaluate'):
    if not 'year' in st.session_state or not 'month' in st.session_state or not 'flat_type' in st.session_state or not 'town' in st.session_state or not 'range_numeric' in st.session_state:
        ...
        with st.spinner('Evaluating...'):
            ...
            # Get prediction using preprocessor and models
            st.session_state['prediction'] = get_price(new_data)

# Display estimated valuation and loan details
...
with c1:
    if 'prediction' in st.session_state:
        ...
        # Sidebar for selecting loan tenure and CPF housing grant
